...
#用于计算当前inci_csn对应的规则
def get_formula_from_incident(inci_csn_list,embedder):
    outputs=[]
    for inci_csn in inci_csn_list:
...
def prediction_to_assignment_embedding(softmax_list, embedder, tokenizers, pres, objs,incident_csn_list):
    """
    ...
    embedded_clauses_list=[]
    for index,incident_csn in enumerate(incident_csn_list):
        embedded_clauses = []

        prop = softmax_list[index]  #softmax是一个向量
        sub = 'subgraph'
        obj = 'subgraph'
        ...
        embedded_clauses_list.append(embedded_clauses)

    return embedded_clauses_list

#给定incident csn号 以及 对应的预测结果给出计算得到的logic loss
def logic_loss(inci_csn_list,prediction_list): #prediction要的是最后一层的输出，是27维的
    formula_embeddings=get_formula_from_incident(inci_csn_list,embedder)

    #info指的就是当前的inci_csn是什么故障类型，(inci_csn,())
    #找到当前inci_csn对应的故障类型：
    assignmen_embeddings=prediction_to_assignment_embedding(F.softmax(prediction_list), embedder, tokenizers, pres, objs,inci_csn_list)

    len=0
    loss_embedding_avg=Variable(torch.Tensor([0.0]), requires_grad=True)
    for index,formula_embedding in enumerate(formula_embeddings):

        loss_embedding=(formula_embedding-assignmen_embeddings[index]).norm()

        len+=1
        loss_embedding_avg=loss_embedding_avg+loss_embedding #torch.Variable变量不能进行+=操作

    return loss_embedding_avg/len #平均loss
if __name__=='__main__':
    ...
    opt = torch.optim.Adam(used_model.parameters())
    ...
    for batched_g, batched_labels in dataloader_train:  # 开始遍历内容,注意不能使用tqdm，我也不知道为什么，反正用了就报错
        ...
        batched_inci_csns_str=[str(inci_csn) for inci_csn in batched_inci_csns]

        logic_loss_value=logic_loss(batched_inci_csns_str,outs) #要保证输入的是str类型的变量

        print(logic_loss_value)

        loss = prediction_loss + 0.1 * logic_loss_value[0]
        opt.zero_grad()
        loss.backward()
        opt.step()
