...
def main():
    ...
    parser.add_argument('--lr', type=float, default=0.00005,     #人工调整学习率~
                        help='Initial learning rate.')
    parser.add_argument('--weight_decay', type=float, default=1e-3,
                        help='Weight decay (L2 loss on parameters).')
    parser.add_argument('--hidden', type=int, default=89,
    ...
    parser.add_argument('--target_dataset_name', type=str, default=0, help='The name of raw dataset, such as Amazon, but not vrd_ddnnf')
    ...
    json.dump(list(file_list_test), open('./model_save/'+target_dataset_name+'/'+dataset + seed_name+'_no_val' + '.testformula', 'w'), ensure_ascii=False)
        ...
        for _, data_train_group in tqdm(enumerate(dataloader_train), desc='Training', total=len(dataloader_train)):
            ...
            for data_train in data_train_group:
                ...
                for data_train_item in data_train:
                    ...
                    adj, features, labels, idx_train, idx_val, idx_test = cuda_input(*data_train_item[:-2]) #这里是对载入的数据转为cuda对象
                ...
                # back prop GCN
                loss_train = creterion(vector3[0].unsqueeze(0), vector3[1].unsqueeze(0), vector3[2].unsqueeze(0))  #这是在计算triplet loss
                                                                                                                   #vector3向量三个维度分别是F,st,sf；规则，满足，不满足
    ...
    for epoch in range(args.epochs):
        ...
        if train_status[2] > best_acc: #自动保存效果最好的embeder
            ...
            torch.save(model,'./model_save/'+target_dataset_name+'/' + dataset + reg_name + indepname + directed_name + cls_reg_name + seed_name +'_no_val'+ '.model')
            torch.save(mlp,'./model_save/' +target_dataset_name+'/' + dataset + reg_name + indepname + directed_name + cls_reg_name + seed_name +'_no_val'+ '.mlp.model')
            print('\tNew best model saved.')
        json.dump(train_loss_save, open('./acc_loss/' +target_dataset_name+'/' + dataset + reg_name + indepname + directed_name + cls_reg_name + seed_name+'_no_val' + '.train_save','w'),ensure_ascii=False)
        pk.dump(loss_by_iter, open('./acc_loss/' +target_dataset_name+'/' + dataset + reg_name + indepname + directed_name + cls_reg_name + seed_name+'_no_val' + '.loss_by_iter','wb'))
