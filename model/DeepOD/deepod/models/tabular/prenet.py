...
from model.DeepOD.deepod.core.networks.base_networks import LinearBlock, MLPnet
import torch
import numpy as np
...
class DualInputNet(torch.nn.Module):
    ...
    def forward(self, x1, x2):
        x1,a = self.enc_net(x1)
        x2,b = self.enc_net(x2)
        pred = self.out_layer(torch.cat([x1, x2], dim=1))
        embeddings = self.out_layer(torch.cat([a, b], dim=1))
