...
from model.DeepOD.deepod.core.base_model import BaseDeepAD
from model.DeepOD.deepod.core.networks.base_networks import get_network
...
class FeaWAD(BaseDeepAD):
    ...
    """
    def __init__(self, epochs=100, batch_size=64, lr=1e-3, input_dim = -1,
                 rep_dim=128, hidden_dims='100,50', hidden_dims2='256,32', act='ReLU', bias=False,
                 margin=5.,
        ...
        network_params = {
            ...
            'n_hidden2': self.hidden_dims2,
            'activation': self.act,
            'bias': self.bias
        }
        self.net = FeaWadNet(**network_params).to(self.device)
        self.criterion = FeaWADLoss(margin=self.margin)
    ...
    def wsad_training_prepare(self, dataset, batch_size):
        # use 1 to denote known anomalies, and 0 to denote known inliers
        X = dataset.X
        y = dataset.y
    ...
    def validation_prepare(self, X, y, batch_size):
        # loader: balanced loader, a mini-batch contains a half of normal data and a half of anomalies
    ...
    def testing_prepare(self, X, y, batch_size):
        # loader: balanced loader, a mini-batch contains a half of normal data and a half of anomalies
    ...
    # we adapt the following implementation to our comparison experiments
    def forward(self, batch_x, batch_y):
        batch_x = batch_x.float().to(self.device)
        batch_y = batch_y.to(self.device)
        pred, sub_result, embeddings = self.net(batch_x)
        return pred, sub_result, embeddings

    # we adapt the following implementation to our comparison experiments
    def inference_score_forward(self, batch_x):
        batch_x = batch_x.float().to(self.device)
        s, _, _ = self.net(batch_x)
        scores = s.view(-1)
        batch_z = batch_x
...
class FeaWadNet(torch.nn.Module):
    ...
    def forward(self, x):
        ...
        if len(concat.shape) == 3:
            concat = concat[:, -1]
        out, embeddings = self.LinearModel(concat)        

        return out, sub_result, embeddings
...
class FeaWADLoss(torch.nn.Module):
    ...
    def forward(self, y_true, y_pred, sub_result):
        ...
        loss = (1 - y_true_cuda) * (inlier_loss + sub_nor) + y_true_cuda * (outlier_loss + outlier_sub_loss)
